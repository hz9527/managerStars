...
export default new Vuex.Store({
  ...
  getters: {
    ...
    getList: (state, getters) => {
      ...
      var result = getters.getAllList.slice()
      if (getters.getTab !== 0) {
        if (getters.getTab === 1) {
          result = result.filter(item => item.tips.length > 0)
        } else if (getters.getTab === 2) {
          result = result.filter(item => item.tips.length === 0)
        }
      }
      if (getters.getLang) {
        result = result.filter(item => item.language === getters.getLang)
      }
      if (getters.getCurTip !== 0) {
        result = result.filter(item => item.tips.some(id => id === getters.getCurTip))
      }
      if (getters.getKeyWord) { // 支持描述 name搜索
        result = result.filter(item => item.full_name.indexOf(getters.getKeyWord) > -1 ||
      ...
      return result.map(item => Object.assign(
        {},
        item,
        {choose: getters.getChooseList.some(id => id === item.id)}
      ))
    },
    ...
    getLangList: (state, getters) => {
      return Array.from(new Set(getters.getAllList.map(item => item.language)))
    }
  },
  mutations: {
